...
const RolePermissions = () => {
    ...
    return (
        <div>
            <h1 className="font-bold text-blue-600 text-xl">Role and Permissions</h1>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {
                                getreols.map((role, index) => {
                                    return (
                                        <tr className="hover:bg-blue-50 transition-all duration-150" key={index}>
                                            <td className="px-6 py-4 font-medium text-gray-800">{index + 1}</td>
                                            <td className="px-6 py-4">{role.name}</td>
                                            <td className="px-6 py-4">
                                                <Link to={`/Dashboard/View-One-Role/${role._id}`} className="text-blue-600 font-medium hover:underline">
                                                    View
                                                </Link>
                                            </td>
                                        </tr>
